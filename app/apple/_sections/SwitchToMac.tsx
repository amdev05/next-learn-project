  ...
  return (
    <section className="relative py-28 bg-[#f5f5f7]">
      ...
      <div className="w-[87.5dvw] mb-7 mx-auto">
        <h2 className="">
          <span className="text-3xl font-semibold">Switch to Mac.</span>
        </h2>
      </div>
          ...
          {data.map((card, index) => (
            <li className="snap-start my-2 hover:scale-103 transition duration-400" key={index}>
              <Link href={""} className="block w-65 bg-white rounded-3xl overflow-hidden">
                <div className="px-6 pt-10 flex flex-col justify-center">
                  ...
                  <p className="text-sm text-center mt-2.5">{card.description}</p>

                  <span className="text-[#007eb4] text-center text-sm mt-2.5 hover:underline">Learn more &gt;</span>
                </div>

                <figure className="relative pb-8">
                  <Image src={card.image} alt="image" width={520} height={558} className="object-cover" />
                </figure>
              </Link>
            </li>
          ))}
